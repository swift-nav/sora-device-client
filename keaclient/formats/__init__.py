...
def format_from_config(config, driver):
  ...
  if len(formats_cfg) != 1:
    raise confuse.exceptions.ConfigValueError('Exactly one format should be specified')

  format_type = list(formats_cfg.keys())[0]
  if format_type not in FORMATS:
    raise confuse.exceptions.ConfigValueError(f'Unknown format type "{format_type}"')
